...
class RecordingManager:
    ...
    async def _run_ytdlp(self, session: RecordingSession, url: str):
        ...
        if ret != 0 and session.status != "stopping":
            ...
            all_err_text = " ".join(session.logs)
            if "Sign in to confirm you’re not a bot" in all_err_text or "bot" in err_msg.lower():
                err_msg = "YouTube Bot Check: YouTube blocked headless extraction for this video. Use 'Web Page (Headless Chromium Tab)' capture or a direct HLS/sports stream."
            session.status = "error"
            session.error_message = err_msg

    async def _run_browser(self, session: RecordingSession, url: str):
        # Browser capture using Playwright with system Google Chrome
        session.logs.append("Launching Google Chrome for Web Page recording...")
        try:
            from playwright.async_api import async_playwright
            ...
            async with async_playwright() as p:
                ...
                # Find recorded webm file in temp_rec_dir and remux/rename to output_path
                ...
                if recorded_files:
                    ...
                    session.logs.append("Warning: No video file was generated by browser context.")
